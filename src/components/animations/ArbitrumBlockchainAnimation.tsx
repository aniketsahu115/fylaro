...
const ArbitrumBlockchainAnimation: React.FC = () => {
  ...
  const colors = {
    ...
    dataBlue: '#4361EE',
    dataBlueDark: '#3249BE',
    networkPurple: '#9747FF',
    networkPurpleDark: '#7435CC',
    background: '#0B0E11',
  ...
  useEffect(() => {
    ...
    const drawAnimatedBackground = (ctx: CanvasRenderingContext2D, width: number, height: number, time: number) => {
      ...
      // Animated grid
      ctx.strokeStyle = `rgba(40, 160, 240, 0.1)`;
      ctx.lineWidth = 1;
      const gridSize = 50;
    ...
    const drawNetworkNodes = (ctx: CanvasRenderingContext2D, time: number) => {
      ...
      nodes.forEach((node, i) => {
        node.connections.forEach(connIndex => {
          ...
          ctx.strokeStyle = `rgba(151, 71, 255, ${0.1 + activity * 0.2})`;
          ctx.lineWidth = 1;
      ...
      nodes.forEach((node, i) => {
        ...
        ctx.fillStyle = `rgba(151, 71, 255, ${0.3 + node.activity * 0.5})`;
        ctx.shadowColor = colors.arbitrumBlue;
    ...
    const drawConnections = (ctx: CanvasRenderingContext2D, time: number) => {
      ...
      for (let i = 0; i < blocks.length - 1; i++) {
        ...
        gradient.addColorStop(0, `rgba(40, 160, 240, ${0.4 + activity * 0.4})`);
        gradient.addColorStop(0.5, `rgba(151, 71, 255, ${0.6 + activity * 0.3})`);
        gradient.addColorStop(1, `rgba(40, 160, 240, ${0.4 + activity * 0.4})`);
        ...
        flowGradient.addColorStop(Math.max(0, flowOffset - 0.1), 'rgba(40, 160, 240, 0)');
        flowGradient.addColorStop(flowOffset, 'rgba(40, 160, 240, 0.8)');
        flowGradient.addColorStop(Math.min(1, flowOffset + 0.1), 'rgba(40, 160, 240, 0)');
    ...
    const drawBlocks = (ctx: CanvasRenderingContext2D, time: number) => {
      blocksRef.current.forEach((block, index) => {
        ...
        // Outer glow
        ctx.shadowColor = colors.arbitrumBlueGlow;
        ...
        ctx.strokeStyle = `rgba(40, 160, 240, ${0.6 + glowIntensity * 0.4})`;
        ctx.lineWidth = 3;
        ...
        if (block.validated && block.validationProgress > 0) {
          ...
          // Draw validation dot
          ctx.fillStyle = `rgba(40, 160, 240, ${block.validationProgress})`;
          ctx.beginPath();
        ...
        ctx.strokeStyle = `rgba(151, 71, 255, ${0.4 + glowIntensity * 0.3})`;
        ctx.lineWidth = 1;
        for (let i = 0; i < 4; i++) {
          const lineAlpha = (Math.sin(time * 0.01 + i) + 1) * 0.5;
          ctx.strokeStyle = `rgba(151, 71, 255, ${lineAlpha * 0.5})`;
          ctx.beginPath();
    ...
    const drawDataParticles = (ctx: CanvasRenderingContext2D, time: number) => {
      dataParticlesRef.current.forEach(particle => {
        ...
        const color = particle.type === 'validation' ? colors.arbitrumBlue :
                     particle.type === 'transaction' ? colors.dataBlue :
                     colors.networkPurple;
        ...
        particle.trail.forEach((point, index) => {
          ...
          ctx.fillStyle = `rgba(${color === colors.arbitrumBlue ? '40, 160, 240' : 
                                 color === colors.dataBlue ? '67, 97, 238' : 
                                 '151, 71, 255'}, ${trailAlpha})`;
          ctx.beginPath();
        ...
        if (particle.type === 'validation') {
          for (let i = 0; i < 4; i++) {
            ...
            ctx.fillStyle = `rgba(40, 160, 240, ${pulse * 0.8})`;
            ctx.beginPath();
